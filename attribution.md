...
|--------|----------------|-------------------------------|
| [MITRE ATT&CK](https://attack.mitre.org/) ([STIX data](https://github.com/mitre-attack/attack-stix-data)) | Group descriptions; ATT&CK IDs; technique/tactic/campaign/mitigation/software pages and actor relationships (`ttps`, `software`, `campaigns`); optional denormalized `attck_techniques` / `attck_software` / `attck_references` for APIs. | MITRE permission notice appears on affected pages and in structured actor metadata for records imported from STIX. |
| [abuse.ch ThreatFox](https://threatfox.abuse.ch/) | Recent community IOC snapshots merged into existing actors when malware names or tags match (`provenance.threatfox`). | ThreatFox data is attributed to abuse.ch; use is subject to [abuse.ch terms](https://abuse.ch/terms-of-use/) and fair-use expectations for the community API. |
| [abuse.ch MalwareBazaar](https://bazaar.abuse.ch/) | Snapshot-backed malware hashes and metadata; only explicit actor evidence may be merged into actor IOCs. | MalwareBazaar is attributed to abuse.ch; terms of use and fair-use limits apply. Malware-family names are not actor attribution. |
| [abuse.ch URLhaus](https://urlhaus.abuse.ch/) | Snapshot-backed malicious URLs and metadata; only explicit actor evidence may be merged into actor IOCs. | URLhaus is attributed to abuse.ch; terms of use and fair-use limits apply. Threat tags are not actor attribution. |
...
| [Orange Cyberdefense Ransomware Ecosystem Map](https://github.com/cert-orangecyberdefense/ransomware_map) | Pinned README changelog mentions and weak PDF extraction signals for manual ransomware-ecosystem review candidates. | The visual map is a secondary, uncertainty-laden source. Upstream PDF binaries are not redistributed; provenance stores the pinned URL/version/commit and SHA-256 hash. Relationships, timelines, aliases, and entity types require analyst adjudication. |
