...
2. Preserve the workstream before cleanup: have its owner commit it on a named branch, or create a patch/bundle plus a tar archive of untracked OCD files and the dirty diff. Record hashes and archive paths outside the repository. Do not use `git stash -u` as the sole backup because it is easy to lose track of untracked/generated material.
