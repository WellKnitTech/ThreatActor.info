---
layout: technique
title: Manipulate I/O Image
mitre_id: T0835
permalink: "/techniques/T0835/"
mitre_url: https://attack.mitre.org/techniques/T0835
domains:
- ics
domain: ics
tactic_ids:
- TA0107
is_subtechnique: false
source_attribution: "© The MITRE Corporation. This work is reproduced and distributed
  with the permission of The MITRE Corporation."
description_excerpt: 'Adversaries may manipulate the I/O image of PLCs through various
  means to prevent them from functioning as expected. Methods of I/O image manipulation
  may include overriding the I/O table via direct memory manipulation or using the
  override function used for testing PLC programs. '
attack_version: '19.0'
---

## Description

Adversaries may manipulate the I/O image of PLCs through various means to prevent them from functioning as expected. Methods of I/O image manipulation may include overriding the I/O table via direct memory manipulation or using the override function used for testing PLC programs. [Dr. Kelvin T. Erickson December 2010](https://www.scribd.com/document/458637574/Programmable-Logic-Controllers) During the scan cycle, a PLC reads the status of all inputs and stores them in an image table. [Nanjundaiah, Vaidyanath](https://www.ezautomation.net/industry-articles/plc-ladder-logic-basics.htm) The image table is the PLCs internal storage location where values of inputs/outputs for one scan are stored while it executes the user program. After the PLC has solved the entire logic program, it updates the output image table. The contents of this output image table are written to the corresponding output points in I/O Modules. 

One of the unique characteristics of PLCs is their ability to override the status of a physical discrete input or to override the logic driving a physical output coil and force the output to a desired status.

### Mitigations

- [Mitigation Limited or Not Effective](/mitigations/M0816/) (`M0816`)


---

*© The MITRE Corporation. This work is reproduced and distributed with the permission of The MITRE Corporation.*
